...
export default class Calendar extends React.Component {
  ...
  render() {
    var month = this.props.monthNames[this.state.month];
    var calendar = this.getCalendarDates(this.state.month, this.state.year);
    var calendarEls = calendar.map((week, i) =>{
  ...
  onNextMonthClick() {
    var m = moment([this.state.year, this.state.month]).add(1, 'month');
    this.setState({month: m.month(), year: m.year()});
  }

  onPrevMonthClick() {
    var m = moment([this.state.year, this.state.month]).subtract(1, 'month');
    this.setState({month: m.month(), year: m.year()});
  }
  ...
  getCalendarDates (month, year) {
    var calendar = [];
    var curDate = this.getFirstDateTuple(month, year);
    for(var i=0;i<6;i++) {
      var week = [];
      for(var j=0;j<7;j++) {
        week.push({
  ...
  getFirstDateTuple(month, year) {
    ...
    var daysInLastMonth = monthAgo.daysInMonth(),
      ...
      firstDate = daysInLastMonth - dayOfWeek + this.props.firstDayOfWeek + 1;
  ...
  isDateInSelectedRange (date) {
    var fromDate = this.getFromDateTuple(),
      toDate = this.getToDateTuple();
    if (!fromDate || !toDate) {
      return false;
    }
  ...
  isDateInMinMax (date) {
    ...
    return this.isDateTupleInRange(date, this.props.minValue, this.props.maxValue);
  }
