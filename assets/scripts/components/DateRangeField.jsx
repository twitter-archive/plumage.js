...
export default class DateRangeField extends React.Component {
  ...
  render() {
    ...
    var fromCalendarValue = this.getFromCalendarValue();
    var toCalendarValue = this.getToCalendarValue();
  ...
  setValues(fromValue, toValue, showPicker) {
    ...
    var oldFromValue = this.state.fromValue;
    var oldToValue = this.state.toValue;
    this.setState({
      fromValue: fromValue,
      toValue: toValue,
    ...
    if (this.props.onModelChange && (oldFromValue !== fromValue || oldToValue !== oldToValue)) {
      var newValues = {};
   ...
   */
  commitText() {
    var newValues = this.parseTextToMoments(this.state.valueText);
  ...
  getFromCalendarValue() {
    if (this.state.textChanged) {
      var values = this.parseTextToMoments(this.state.valueText);
      if (values[0].isValid()) {
  ...
  selectPresetRange(range) {
    ...
    var today = this.props.utc ? moment.utc().startOf('day') : moment().startOf('day');
  ...
  onRangeClick(e) {
    e.preventDefault();
    e.stopPropagation();
    var range = _.findWhere(this.props.ranges, {name: e.target.text});
